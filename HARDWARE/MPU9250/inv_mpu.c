...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
		i2c_read(st.hw->addr, st.reg->pwr_mgmt_1,1, gSensor.imuData.imu_Reg_context);
		printf("PWR_MGMT_reg = %x\r\n", gSensor.imuData.imu_Reg_context[0]);
    delay_ms(100);
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
		if(i2c_read(st.hw->addr, st.reg->accel_cfg2,1, gSensor.imuData.imu_Reg_context))
			printf("error: fail to get data from MPU\r\n");
		else
			printf("PWR_MGMT_reg = %x\r\n", gSensor.imuData.imu_Reg_context[0]);
#endif
    ...
    unsigned char *more)
{
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, tmp)){
					printf("Debug :fail to read the FIFO overflow bit\r\n");
            return -1;
					
				}
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
					printf("Debug : FIFO overflows and being reset\r\n");
            return -2;
        }
    }
